...
if target.alive and target.enemy and player.alive and not player.channeling() then
  ...
  -- Healing
    ...
    if (castable(SB.Execute) or -buff(SB.SuddenDeath)) and -power.rage >= 20 then
    ...
    -- Cast Slam
    if castable(SB.Slam) and -spell(SB.Slam) == 0 and -power.rage >= 20 then
