...
if target.alive and target.enemy and player.alive and not player.channeling() then
  ...
  -- Healing
    ...
    if castable(SB.Rejuvenation) and -spell(SB.Rejuvenation) == 0 and -player.health <= 35 and player.buff(SB.Rejuvenation).down then
        print('Rejuvenation @ ' .. -player.health)
        return cast(SB.Rejuvenation)
    end
    if castable(SB.Swiftmend) and -spell(SB.Swiftmend) == 0 and -player.health <= 30 then
        print('Swiftmend @ ' .. -player.health)
        return cast(SB.Swiftmend)
    end          
    --Survival Instincts
    if castable(SB.SurvivalInstincts) and -spell(SB.SurvivalInstincts) == 0 and -player.health <= 20 then
        print('Survival Instincts @ ' .. -player.health)
  ...
  -- Remove Curse and Poison
    ...
    -- local unit = player.dispellable(SB.RemoveCorruption)
    -- if unit and unit.castable(SB.RemoveCorruption) then
    --   return cast(SB.RemoveCorruption, unit)
    -- end    
  ...
  -- Apply Cat Form IC
    ...
    if castable(SB.WorldveinResonance) and -spell(SB.WorldveinResonance) == 0 and target.distance < 5 then
        ...
        -- --Cast Ferocious Bite to extend Rip if already active.
        ...
        and (target.debuff(SB.Rip).down or target.debuff(SB.Rip).remains < 7.5) then
            return cast(SB.Rip)
        end

        --Maintain Rake
        if castable(SB.Rake, 'target') and target.debuff(SB.Rake).remains < 4 then --and -spell(SB.Rake) == 0
            return cast(SB.Rake)
        end
        ...
        --Cast Swipe to build Combo Points a 3+ targets
...
local function interface()
    ...
    local settings = {
        ...
        template = {
            ...
            { type = 'text', text = 'Suggested PVE Talents - 1 3 1 1 2 1 1' },
