...
if target.alive and target.enemy and player.alive and not player.channeling() then
  ...
  -- Use Racials
    ...
    if enemyCount >= 2 or toggle('multitarget', false) and target.distance <= 8 then
        ...
        if castable(SB.Whirlwind) and -spell(SB.Whirlwind) == 0 and player.buff(SB.Whirlwind).count < 2 then
    ...
    -- Top talents: Endless Rage, Furious Slash, Carnage, Dragon Roar, Siegebreaker
        ...
        if castable(SB.Rampage) and -spell(SB.Rampage) == 0 and (player.buff(SB.Enrage).down or -power.rage > 90) then
        ...
        if castable(SB.Execute) and -spell(SB.Execute) == 0 then
            return cast(SB.Execute)
        end

        -- Raging Blow at 2 charges and Enraged
        if castable(SB.RagingBlow) and -spell(SB.RagingBlow) == 0 and spell(SB.RagingBlow).charges == 2 then
...
local function resting()
  ...
  --Battle Shout OOC
  if castable(SB.BattleShout) and -spell(SB.BattleShout) == 0 and not player.buff(SB.BattleShout).up then
