...
local function combat()
  ...
  local kickInt = dark_addon.settings.fetch('outrog_settings_kickInt', true)
  local gougeInt = dark_addon.settings.fetch('outrog_settings_gougeInt', true)
  local blindInt = dark_addon.settings.fetch('outrog_settings_blindInt', true)      
  ...
  local useessences = dark_addon.settings.fetch('outrog_settings_useessences', true)
  ...
  --Cast Between the Eyes at 4-5 Combo Points if you have a Ruthless Precision proc, or Ace Up Your Sleeve , or Deadshot
  ...
  --Cast Dispatch at 4-5 Combo Points.
  if castable(SB.Dispatch) and -spell(SB.Dispatch) == 0 and player.power.combopoints.actual >= 4 then
    return cast(SB.Dispatch, 'target')
  end

  --Cast Pistol Shot if you have an Opportunity proc and you have 3 or less Combo Points (and will not Energy cap during the global cooldown).
...
local enemyCount = enemies.around(8)
dark_addon.interface.status_extra('T#:' .. enemyCount .. ' D:' .. target.distance)
local stealthooc = dark_addon.settings.fetch('outrog_settings_stealthooc', true)

--Stealth OOC
if castable(SB.Stealth) and -spell(SB.Stealth) == 0 and player.buff(SB.Stealth).down and stealthooc then 
...
local function interface()
    ...
    local settings = {
        ...
        template = {
            ...
            { key = 'smartrtb', type = 'checkbox', text = 'Smart Roll the Bones', desc = '', default = true },
            { key = 'stealthooc', type = 'checkbox', text = 'Auto Stealth Out of Combat', desc = '', default = true },
