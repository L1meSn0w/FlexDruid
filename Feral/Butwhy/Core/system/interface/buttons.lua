...
fontObject:SetFont("Interface\\Addons\\Feral\\Butwhy\\Core\\media\\OpenSans-Regular.ttf", button_size / 4)

fontObject = CreateFont("dark_addon_small")
fontObject:SetFont("Interface\\Addons\\Feral\\Butwhy\\Core\\media\\OpenSans-Regular.ttf", 12)

fontObject = CreateFont("dark_addon_bold")
fontObject:SetFont("Interface\\Addons\\Feral\\Butwhy\\Core\\media\\OpenSans-Bold.ttf", button_size / 4)

fontObject = CreateFont("dark_addon_icon")
fontObject:SetFont("Interface\\Addons\\Feral\\Butwhy\\Core\\media\\FontAwesomeProRegular.otf", button_size / 2)
...
function dark_addon.interface.status_override(text, duration, colorA, colorB)
  ...
  if not colorA or not colorB then
    colorA = dark_addon.interface.color.blue
    colorB = dark_addon.interface.color.dark_grey
  end
  do
  ...
  C_Timer.After(duration, function()
    do
      local minR, minG, minB = dark_addon.interface.color.hexToRgb(dark_addon.interface.color.dark_blue)
      local maxR, maxG, maxB = dark_addon.interface.color.hexToRgb(dark_addon.interface.color.dark_grey)
      info_frame.gradient:SetColorTexture(1, 1, 1, 0.75)
      info_frame.gradient:SetGradient('VERTICAL', maxR, maxG, maxB, minR, minG, minB)
    end
  ...
  dark_addon.interface.buttons.add({
    ...
    toggle_on = function(self)
      ...
      if button.label then
        dark_addon.interface.status_override(button.label .. ' Опять работать?', 1)
      end
    end,
    ...
    toggle_off = function(self)
      ...
      if button.label then
        dark_addon.interface.status_override(button.label .. ' отключено.', 1)
      end
    end,
...
dark_addon.on_ready(function()
  ...
  dark_addon.commands.register({
    command = 'move',
    arguments = { },
    text = 'Блокирует и разблокирует перемещение панели.',
  ...
  C_Timer.After(2, function()
    dark_addon.interface.status_override(
      'Напиши /fd help и узнаешь как влючить всё.',
      5
    )
  end)
  else 
    C_Timer.After(2, function()
    dark_addon.interface.status_override(
      'Type /fd help and get all commands.',
      5
    )
  end)
  end
