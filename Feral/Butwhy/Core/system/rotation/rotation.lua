...
function dark_addon.rotation.load(name)
  ...
  if rotation then
    ...
    dark_addon.log('Loaded: ' .. name)
    dark_addon.interface.status('Ready...')
  else
    dark_addon.error('Try Re-Load or not found: ' .. name)
  end
else
 if rotation then
    dark_addon.settings.store('active_rotation_' .. dark_addon.rotation.current_spec, name)
    ...
    dark_addon.log('Загружено: ' .. name)
    dark_addon.interface.status('Готово...')
  else
    dark_addon.error('Не найдено|перезагрузи: ' .. name)
  end
end
end
