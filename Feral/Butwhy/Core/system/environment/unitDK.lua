...
local function casting(spell)
  local casting_spell = UnitCastingInfo("Target")
...
--Check deBuff on Unit
function havedeBuff(unit, debuff)
    for i=1,40 do
        local _,_,_,_,_,_,_,_,_,spell_id = UnitDebuff(unit,i)
        if spell_id == nil then
            break
        end
        if spell_id == debuff then
            return true
        end
    end
    return false
...
function amitargeted()
  if UnitName("targettarget") == name or not UnitName("targettarget") == pet then
    return true
  end
end
...
local function unit_interrupt(percent, spell)
  ...
  local name, startTime, endTime, isTradeSkill, notInterruptible
  name, _, _, startTime, endTime, _, _, notInterruptible, _ = UnitCastingInfo(calledUnit.unitID)
  if not name then
    name, _, _, startTime, endTime, isTradeSkill, notInterruptible, _ = UnitChannelInfo(calledUnit.unitID)
  end
  if name and not isTradeSkill and startTime and endTime and ((spell and name == spell) or (not spell) and not notInterruptible) then
