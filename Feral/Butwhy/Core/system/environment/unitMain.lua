...
local dr_buildinfo = GetBuildInfo()
...
local function channeling(spell)
  if dr_buildinfo == "1.13.4" then 
   channeling_spell = ChannelInfo(unit.calledUnit.unitID)
  end
  if dr_buildinfo == "8.3.0" then
   channeling_spell = UnitChannelInfo(unit.calledUnit.unitID) 
  end
...
function unit:casting()
  ...
  local castingname, _ = UnitCastingInfo("player")
  end
...
local function unit_interrupt(percent, spell)
  ...
  if not name then
    name, _, _, startTime, endTime, _, notInterruptible, _ = UnitChannelInfo(unit.calledUnit.unitID)
  end
...
local function spell_castable(spell)
  ...
  local inRange = IsSpellInRange(spell, unit.calledUnit.unitID)
  local onCooldown = false
  if dark_addon.healthCooldown[unit.calledUnit.unitID] ~= nil
  and dark_addon.healthCooldown[unit.calledUnit.unitID] > GetTime() then
    onCooldown = true
  end
  local unitid = UnitName(unit.calledUnit.unitID)
  if unitid == nil then unitid = 'nil' end
  dark_addon.console.debug(1, 'engine', 'engine', string.format('in unit:castable unit %s onCooldown %s', unitid, tostring(onCooldown)))
